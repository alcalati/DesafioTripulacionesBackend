...
export const register = async (req, res) => {
  try {
    ...
    const emailToken = jwt.sign({ email }, ${process.env.JWT_SECRET_KEY}_SECRET, { expiresIn: '48h' });
...
export const confirmEmail = async (req, res) => {
  try {
    ...
    // Verificar el token
    const decoded = jwt.verify(emailToken, ${process.env.JWT_SECRET_KEY}_SECRET);
